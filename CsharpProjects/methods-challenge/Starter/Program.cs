...
while (!shouldExit)
{
    ...
    else
    {
        if (GoodFood())
        {
            // Speed boost on good food
            Move(speed: 3);
        }
        else if (BadFood())
        {
            // Freeze movement on bad food
            FreezePlayer();
        }
        else
        {
...
void Move(int speed = 1, bool allowTerminate = false)
{
    int lastX = playerX;
    int lastY = playerY;
    ...
    if (allowTerminate)
    {
        EndGame("Please use directional keys only");
        return;
    }
...
void TerminateOnResize()
{
    if (TerminalResized())
    {
        EndGame("Console was resized. Program exiting.");
    }
}
